...
{
	if (WindAudioComponent)
		WindAudioComponent->PitchMultiplier = FMath::Clamp(0.f, 1.f, speed / 10000.f);

	if (SlideAudioComponent)
		SlideAudioComponent->Activate(grounded);
}

void USledSound::PlayHitSounds(TArray<FString> tags)
{
	if (tags.Find("Rock") && RockHitAudioComponent)
	{
		RockHitAudioComponent->Play();
	}
	else if (tags.Find("Tree") && TreeHitAudioComponent)
	{
